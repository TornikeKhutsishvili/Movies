...
export class FilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.movieService.getNewTitlesWithPosters().subscribe(data => {
      ...
      // პირველადი ფიქსაცია - ყველა ფილმი იწოდება filtered-ში
      this.movieFilterService.setFilteredMovies(this.movies);
    });
  }

  applyFilter() {
    const { year, genre, rating, language, minRuntime, maxRuntime } = this.filterForm.value;
    ...
    const filtered = this.movies.filter(movie => {
      ...
      const matchMinRuntime = minRuntime ? movie.runtime_minutes >= minRuntime : true;
      const matchMaxRuntime = maxRuntime ? movie.runtime_minutes <= maxRuntime : true;
